...
func TestTokenStoreRejectsCatalogGrantAfterProviderURLChanges(t *testing.T) {
	...
	store := &tokenStore{gatewayClient: client, userID: "user-1", mcpID: mcpID, mcpURL: oldURL}
	_, _, err := store.GetTokenConfig(t.Context())
...
func TestTokenStoreRejectsLegacyGrantWithoutCatalogFence(t *testing.T) {
	...
	store := &tokenStore{gatewayClient: client, userID: "user-1", mcpID: mcpID, mcpURL: mcpURL}
	_, _, err := store.GetTokenConfig(t.Context())
	require.ErrorIs(t, err, gateway.ErrMCPOAuthCatalogCredentialChanged)
}
...
func TestTokenStoreReadWaitsForCatalogMutationFence(t *testing.T) {
	...
	go func() {
		_, _, err := (&tokenStore{gatewayClient: client, userID: "user-1", mcpID: mcpID, mcpURL: mcpURL}).GetTokenConfig(t.Context())
		result <- err
	}()
	select {
	case err := <-result:
...
func TestTokenStoreRefreshCannotResurrectCatalogGrantAfterAppChange(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			store := &tokenStore{gatewayClient: client, userID: "user-1", mcpID: mcpID, mcpURL: mcpURL}
			config, _, err := store.GetTokenConfig(t.Context())
			require.NoError(t, err)
			...
			err = store.SetTokenConfig(t.Context(), config, &oauth2.Token{AccessToken: "refreshed-access", RefreshToken: "refresh-1"})
			require.ErrorIs(t, err, gateway.ErrMCPOAuthCatalogCredentialChanged)
...
func TestTokenStoreRejectsLegacyStaticGrantLeftByFailedClear(t *testing.T) {
	...
	require.NoError(t, client.DeleteMCPOAuthTokenForAllUsers(t.Context(), mcpID))
	err = store.SetTokenConfig(t.Context(), config, &oauth2.Token{AccessToken: "resurrected-access"})
	require.ErrorIs(t, err, gateway.ErrMCPOAuthCatalogCredentialChanged)
	_, err = client.GetMCPOAuthToken(t.Context(), "user-1", mcpID, mcpURL)
	require.ErrorIs(t, err, gorm.ErrRecordNotFound)
}
...
func TestTokenStorePreservesLegacyDynamicGrantRefresh(t *testing.T) {
	...
	store := &tokenStore{gatewayClient: client, userID: "user-1", mcpID: mcpID, mcpURL: mcpURL}
	loadedConfig, _, err := store.GetTokenConfig(t.Context())
	require.NoError(t, err)
	require.NoError(t, store.SetTokenConfig(t.Context(), loadedConfig,
...
func newCatalogTokenStoreTestClientWithStorage(t *testing.T, entryName, mcpID string, staticOAuthRequired bool) (*gateway.Client, kclient.Client) {
	...
	client := gateway.New(t.Context(), db, storage, nil, nil, nil, nil, time.Hour, 10, 90, 90, 90, true)
